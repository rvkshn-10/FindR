...
Future<AiQueryEnhancement?> enhanceSearchQuery(String userQuery) async {
  try {
    final prompt = '''You are a search assistant for FindR, an app that finds nearby stores using OpenStreetMap Overpass API.
...
Future<AiResultSummary?> generateResultSummary({
  ...
  try {
    ...
    final prompt = '''You are FindR's AI assistant. The user searched for "$query" and found these nearby stores:
