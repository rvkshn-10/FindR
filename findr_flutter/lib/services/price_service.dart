...
Future<PriceData?> fetchProductPrices(String query, {double? lat, double? lng}) async {
  if (kSerpApiKey.isEmpty) {
    debugPrint('[Wayvio] SerpApi: no API key, skipping price lookup');
    return null;
  }
  ...
  try {
    ...
    if (results == null || results.isEmpty) {
      debugPrint('[Wayvio] SerpApi: no shopping results for "$query"');
      return null;
    }

    return _parsePrices(results);
  } catch (e) {
    debugPrint('[Wayvio] SerpApi price fetch failed: $e');
    return null;
  }
}
